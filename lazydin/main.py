...
from lazydin.subprocess_manager import TaskManager
from lazydin.workflows_analyzer import WorkflowsAnalyzer
...
# Find the template directory relative to the package
package_dir = Path(__file__).parent
templates = Jinja2Templates(directory=str(package_dir / "template"))


@asynccontextmanager
async def lifespan(app: FastAPI):
    # Initialize TaskManager with the workflows path
    TaskManager.initialize(workflows_path="workflows")
...
async def get_workflows_documentation(workflows_path: Optional[str] = None):
    ...
    path = workflows_path or "workflows"
